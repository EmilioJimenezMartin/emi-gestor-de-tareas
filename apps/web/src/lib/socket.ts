...
type ServerToClientEvents = {
  ...
  "catalog:progress": (payload: { catalogId: string; status: string; current: number; total: number; image?: { publicId: string; url: string; width: number; height: number; bytes: number; createdAt: string }; imageStartedAt?: number }) => void;
